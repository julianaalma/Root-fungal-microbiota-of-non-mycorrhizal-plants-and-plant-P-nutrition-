...
setwd("C:/Users/pbruyant/Documents/Article thèse/33P experiments dijon")
...
####Second experiment 15N in Arabis alpina#####

###Read data#####
res_P<- read_excel("Pauline-CN15N_2eme_exp.xlsx")
res_P<-filter(res_P, Species=="A. alpina")

##N concentration µg/mg DW ####
###mg/g = µg/mg
res_P$Nconc<-res_P$`Quantité de N totale (mg de N)`/res_P$`Dry weight g without craft`

res_P<-filter(res_P,`Sample type`=="Shoot")
ShootN_conc<-res_P %>% 
  rstatix::reorder_levels(.,Inoculation,c("NI","strain 1-39","strain 1-40","AMF","AMF_plante_nurse")) %>%
  #rstatix::reorder_levels(.,Species,c("A. alpina","M. verna","C. sempervirens","A. ampeloprasum")) %>%
  ggplot(.,aes(y=Nconc, x= Inoculation,color=Inoculation))+
  geom_boxplot(outlier.shape=NA)+
  geom_jitter(aes(color=Inoculation),width = 0.2, alpha=0.3, size=1) + 
  ylab(expression("Shoot N conc. (mg/g DW)"))+xlab("Inoculation")+
  ...
  scale_color_manual(values=c("grey", "#5fbcd3ff","#2c89a0ff","#e0a0b9","#C07F80"))+
  scale_fill_manual(values=c("grey", "#5fbcd3ff","#2c89a0ff","#e0a0b9","#C07F80"))+
  scale_y_continuous(expand = expansion(mult=c(0,0.25)))
ShootN_conc

####Statistics shoot N conc. ####
test<-pairwise.wilcox.test(res_P$Nconc,res_P$Inoculation,p.adjust.method="none")
library(rcompanion)
library(multcompView)
FullMatrix = fullPTable(test$p.value) 
multcompLetters(FullMatrix)
...
ShootN_tot<-res_P %>% 
  rstatix::reorder_levels(.,Inoculation,c("NI","strain 1-39","strain 1-40","AMF","AMF_plante_nurse")) %>%
  #rstatix::reorder_levels(.,Species,c("A. alpina","M. verna","C. sempervirens","A. ampeloprasum")) %>%
  ggplot(.,aes(y=`Quantité de N totale (mg de N)`, x= Inoculation,color=Inoculation))+
  geom_boxplot(outlier.shape=NA)+
  geom_jitter(aes(color=Inoculation),width = 0.2, alpha=0.3, size=1) + 
  ylab(expression("Total shoot N (mg)"))+xlab("Inoculation")+
...
####Statistics total shoot ####
test<-pairwise.wilcox.test(res_P$`Quantité de N totale (mg de N)`,res_P$Inoculation,p.adjust.method="fdr")
FullMatrix = fullPTable(test$p.value) 
multcompLetters(FullMatrix)

## 15N per shoot sample (mg) ####

res_P$N15tot<-res_P$`Quantité de 15N totale (mg de 15N)`

Shoot15N<-res_P %>% 
  rstatix::reorder_levels(.,Inoculation,c("NI","strain 1-39","strain 1-40","AMF","AMF_plante_nurse")) %>%
  #rstatix::reorder_levels(.,Species,c("A. alpina","M. verna","C. sempervirens","A. ampeloprasum")) %>%
  ggplot(.,aes(y=N15tot, x= Inoculation,color=Inoculation))+
  geom_boxplot(outlier.shape=NA)+
  geom_jitter(aes(color=Inoculation),width = 0.2, alpha=0.3, size=1) + 
  ylab(expression("Total shoot 15N (mg)"))+xlab("Inoculation")+
...
####Statistics shoot 15N ####
test<-pairwise.wilcox.test(res_P$N15tot,res_P$Inoculation,p.adjust.method="fdr")
FullMatrix = fullPTable(test$p.value) 
multcompLetters(FullMatrix)

###15N concentration in shoots#####

res_P$N15conc<-res_P$`Quantité de 15N totale (mg de 15N)`/res_P$`Dry weight g without craft`

Shoot15N_conc<-res_P %>% 
  rstatix::reorder_levels(.,Inoculation,c("NI","strain 1-39","strain 1-40","AMF","AMF_plante_nurse")) %>%
  #rstatix::reorder_levels(.,Species,c("A. alpina","M. verna","C. sempervirens","A. ampeloprasum")) %>%
  ggplot(.,aes(y=N15conc, x= Inoculation,color=Inoculation))+
  geom_boxplot(outlier.shape=NA)+
  geom_jitter(aes(color=Inoculation),width = 0.2, alpha=0.3, size=1) + 
  ylab(expression("15N mg/g shoot DW"))+xlab("Inoculation")+
...
####Statistics shoot 15N ####
test<-pairwise.wilcox.test(res_P$N15conc,res_P$Inoculation,p.adjust.method="fdr")
FullMatrix = fullPTable(test$p.value) 
multcompLetters(FullMatrix)
...
biomass<-res_P %>% 
  rstatix::reorder_levels(.,Inoculation,c("NI","strain 1-39","strain 1-40","AMF","AMF_plante_nurse")) %>%
  #rstatix::reorder_levels(.,Species,c("A. alpina","M. verna","C. sempervirens","A. ampeloprasum")) %>%
  ggplot(.,aes(y=`Dry weight g without craft`*1000, x= Inoculation,color=Inoculation))+
  geom_boxplot(outlier.shape=NA)+
  geom_jitter(aes(color=Inoculation),width = 0.2, alpha=0.3, size=1) + 
  ylab(expression("Shoot biomass (mg DW)"))+xlab("Inoculation")+
...
####Statistics in Shoot biomass#####
test<-pairwise.wilcox.test(res_P$`Dry weight g without craft`,res_P$Inoculation,p.adjust.method="fdr")
FullMatrix = fullPTable(test$p.value) 
multcompLetters(FullMatrix)

####Final figure###
library(cowplot)
plot_grid(biomass,Shoot15N_conc, Shoot15N, ShootN_tot, ShootN_conc, nrow = 1)
